    ...
    for game in games:
        ...
        game_summaries.append(
            GameSummary(
                id=game.id,
                name=(
                    opposing_deck.owner_name
                    if not opposing_deck.is_ai_deck
                    else opposing_deck.name
                ),
                type=game.type,
...
def create_game(request):
    ...
    if not player_deck_id:
        ...
        return Response(
            {"error": "player_deck_id is required"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    try:
        ...
        if ai_deck_id:
            ...
            opponent_deck = get_object_or_404(
                Deck, id=ai_deck_id, ai_player__isnull=False
            )
            ...
            opponent_deck = get_object_or_404(
                Deck, id=opponent_deck_id, user__isnull=False
            )
        ...
        if player_deck.title != opponent_deck.title:
            logger.error(
                f"Title mismatch: player deck title {player_deck.title} != opponent deck title {opponent_deck.title}"
            )
            return Response(
...
def queue_for_ranked_match(request):
    ...
    if not deck_id:
        ...
        return Response(
            {"error": "deck_id is required"}, status=status.HTTP_400_BAD_REQUEST
        )
        ...
        return Response(
            {
                ...
                "title": {"slug": deck.title.slug, "name": deck.title.name},
                "deck": {"id": deck.id, "name": deck.name, "hero": deck.hero.name},
                "elo_rating": user_rating.elo_rating,
                "ladder_type": ladder_type,
...
def leave_matchmaking_queue(request, title_slug):
    ...
    logger.info(
        f"User {request.user.display_name} left matchmaking queue for {title.name}"
    )
...
def create_friendly_challenge(request):
    ...
    if not (title_slug and challengee_user_id and challenger_deck_id):
        return Response(
            {
                "error": "title_slug, challengee_user_id, and challenger_deck_id are required"
            },
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    challenger_deck = get_object_or_404(
    ...
    if challengee.id == request.user.id:
        return Response(
            {"error": "Cannot challenge yourself"}, status=status.HTTP_400_BAD_REQUEST
        )
        ...
        return Response(
            {
                ...
                "challengee": {
                    "id": challengee.id,
                    "display_name": challengee.display_name,
                },
                "challenger_deck": {
                    "id": challenger_deck.id,
                    "name": challenger_deck.name,
                    "hero": challenger_deck.hero.name,
                },
            },
            status=status.HTTP_200_OK,
        )
    ...
    incoming_payload = [
        {
            ...
            "challenger": {
                "id": c.challenger.id,
                "display_name": c.challenger.display_name,
            },
            "challengee": {
                "id": c.challengee.id,
                "display_name": c.challengee.display_name,
            },
            "challenger_deck": {
                "id": c.challenger_deck.id,
                "name": c.challenger_deck.name,
                "hero": c.challenger_deck.hero.name,
            },
...
def accept_friendly_challenge(request, challenge_id):
    ...
    if challenge.challengee != request.user:
        return Response(
            {"error": "Only the challengee can accept this challenge"},
            status=status.HTTP_403_FORBIDDEN,
        )
    ...
    if not challengee_deck_id:
        return Response(
            {"error": "challengee_deck_id is required"},
            status=status.HTTP_400_BAD_REQUEST,
        )

    # Validate deck ownership and title
    challengee_deck = get_object_or_404(
...
def decline_friendly_challenge(request, challenge_id):
    ...
    if challenge.challengee != request.user:
        return Response(
            {"error": "Only the challengee can decline this challenge"},
            status=status.HTTP_403_FORBIDDEN,
        )
