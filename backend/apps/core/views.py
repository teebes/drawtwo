...
from .schemas import Deck as DeckSchema
...
@api_view(["GET"])
def titles(request):
    titles = Title.objects.filter(status=Title.STATUS_PUBLISHED).order_by(
        "-published_at"
    )
...
def title_cards(request, slug):
    ...
    cards_queryset = CardTemplate.objects.filter(title=title, is_latest=True).order_by(
        "cost", "card_type", "attack", "health", "name"
    )
...
def title_card_detail(request, slug, card_slug):
    ...
    card_data = serialize_cards_with_traits(CardTemplate.objects.filter(pk=card.id))
...
def title_heroes(request, slug):
    ...
    heroes = HeroTemplate.objects.filter(title=title, is_latest=True).order_by("name")
    hero_data = [
        Hero(
            id=hero.id,
            slug=hero.slug,
...
def title_games(request, slug):
    ...
    if request.user.is_authenticated:
        games = (
            Game.objects.where_user_is_side(title, request.user)
...
def title_games_history(request, slug):
    ...
    ended_games = (
        Game.objects.for_title(title)
        .for_user(user)
        .filter(status=Game.GAME_STATUS_ENDED)
        .select_related(
            "side_a__user",
            "side_b__user",
            "side_a__hero",
    ...
    all_games = (
        ...
        .filter(status__in=[Game.GAME_STATUS_ENDED, Game.GAME_STATUS_IN_PROGRESS])
        .select_related(
            "side_a__user",
            "side_b__user",
            "side_a__hero",
